...
void empaquetar_32 (unsigned numero, void *resultado) {
  unsigned char posicion;
  for (posicion = 0; posicion < 4; posicion ++) { // guardo el ultimo byte y luego lo descarto.
    ((char *) resultado)[posicion] = numero & 0xff; //me quedo con el byte de menor peso
    numero >>= 8; //desplaza 8 bits a la der, es decir tira el ultimo byte a la mierda
  }
}
